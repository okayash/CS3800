 ...
 * S	- kernel status field, p_rts_flags; mm status field, mp_flags (R if
 *p_rts_flags is 0; Z if mp_flags == ZOMBIE; T if mp_flags == STOPPED; else W).
...
_PROTOTYPE(int addrread,
           (int fd, phys_clicks base, vir_bytes addr, char *buf, int nbytes));
...
Dev_t dev_nr;
{
  ...
  if (majdev(dev_nr) == TTY_MAJ && mindev(dev_nr) == 0)
    return "co";
...
char *argv[];
{
  ...
  for (i = 1; i < argc; i++) {
    ...
    while (*opt != 0)
      ...
      case 's':
        ...
        //code to get selected state, selected state must be in S/Z/etc form something arg[3] = (a variable) else  { return -1; }
        break; 
      */
      default:
        usage(argv[0]);
  ...
  if (addrread(kmemfd, (phys_clicks)0, proc_addr, (char *)ps_proc,
  ...
  if (addrread(memfd, ps_proc[nr_tasks + PM_PROC_NR].p_memmap[D].mem_phys,
               mproc_addr, (char *)ps_mproc, nr_procs * sizeof(ps_mproc[0])) !=
  ...
  if (addrread(memfd, ps_proc[nr_tasks + FS_PROC_NR].p_memmap[D].mem_phys,
               fproc_addr, (char *)ps_fproc, nr_procs * sizeof(ps_fproc[0])) !=
  ...
  for (i = FS_PROC_NR; i < nr_procs; i++) {
    if (strcmp(ps_proc[nr_tasks + i].p_name, "init") == 0)
      break;
  }
  init_proc_nr = i;
  ...
  for (i = -nr_tasks; i < nr_procs; i++) { //it prints each individually
    if (pstat(i, &buf) != -1 &&
      ...
      if (opt_long)
        printf(L_FORMAT, buf.ps_flags, buf.ps_state, buf.ps_euid, pid,
               buf.ps_ppid, buf.ps_pgrp,
               off_to_k(
                   (buf.ps_tsize + buf.ps_stack - buf.ps_data + buf.ps_ssize)),
               (buf.ps_flags & RECEIVING ? prrecv(&buf) : ""),
               tname((Dev_t)buf.ps_dev), cpu,
               i <= init_proc_nr || buf.ps_args == NULL ? taskname(i)
                                                        : buf.ps_args);
      else
        printf(S_FORMAT, pid, tname((Dev_t)buf.ps_dev), cpu,
               i <= init_proc_nr || buf.ps_args == NULL ? taskname(i)
                                                        : buf.ps_args);
    }
  }
...
int gettynames() {
  ...
  endttyent();
  while (index < n_ttyinfo)
    ttyinfo[index++].tty_dev = 0;
